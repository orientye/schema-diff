...
def config_option():
    ...
    parser.add_option("-o", "--out", dest="diff_schema", help="output schema diff")

    (options, args) = parser.parse_args()

    if not options.from_schema or not options.to_schema or not options.diff_schema:
    ...
    opt_main["diff_schema"] = options.diff_schema
...
class SchemaObjects(object):
    ...
    def run(self):
        ...
        for table in self.diff_tables:
            ...
            self.return_objects['tables'][table]['from_table'] = self._get_table_definitions(
                self.diff_tables[table]['from_table'])
            self.return_objects['tables'][table]['to_table'] = self._get_table_definitions(
                self.diff_tables[table]['to_table'])
    ...
    def _get_table_definitions(self, schema_table):
        ...
        for definition in table_definitions:
            ...
            if column_name:
                return_definitions['column'][column_name.group(2)] = re.match(r"([^`]*)([^,]*)(,?)", definition).group(
                    2)

            primary_name = re.match(r"(\s*PRIMARY KEY\s*)", definition)
            if primary_name:
                return_definitions['primary']['primary'] = re.match(r"(\s*)(PRIMARY KEY \(.*\))(,?)", definition).group(
                    2)

            unique_name = re.match(r"(\s*UNIQUE KEY \`)([^`]*)(\`.*)", definition)
            if unique_name:
                return_definitions['unique'][unique_name.group(2)] = re.match(r"(\s*)(UNIQUE KEY.*\))(,?)",
                                                                              definition).group(2)
            ...
            if foreign_name:
                return_definitions['foreign'][foreign_name.group(2)] = re.match(r"(\s*)(CONSTRAINT[^,]*)(,?)",
                                                                                definition).group(2)

            fulltext_name = re.match(r"(\s*FULLTEXT KEY \`)([^`]*)(\`.*)", definition)
            if fulltext_name:
                return_definitions['fulltext'][fulltext_name.group(2)] = re.match(r"(\s*)(FULLTEXT KEY.*\))(,?)",
                                                                                  definition).group(2)
...
class SchemaAlters(object):
    ...
    def _fulltext(self, table, from_fulltext, to_fulltext):
        ...
        for definition in to_fulltext:
            if definition in from_fulltext:
                pass
            else:
                self._record_alters("alter table `%s` add %s;" % (table, to_fulltext[definition]))

    def _option(self, table, from_option, to_option):
        if 'option' in from_option:
            if 'option' in to_option:
...
def main():
    ...
    diff_schema = open(opt_main["diff_schema"], 'w')
    diff_schema.write(objects_alters)
    diff_schema.write(definitions_alters)
    diff_schema.close()
